...
async function generateStaticPost(post) {
  try {
    ...
    if (bodyEl) {
      let content = post.content || generateDemoContent(post);
    ...
    const slug = post.slug || post.title.toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/(^-|-$)/g, '');
    const filename = `${slug}.html`; // ADD .html extension here
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">`;
  ...
  posts.forEach(post => {
    if (post && post.filename) {
      // Since filename now includes .html, we keep it as is for the sitemap
      // but we want to show URLs without .html extension for clean URLs
      const cleanFilename = post.filename.replace('.html', '');
      sitemap += `
  <url>
    <loc>${baseUrl}${cleanFilename}</loc>
...
// Convert plain text to HTML with proper formatting for blog content
